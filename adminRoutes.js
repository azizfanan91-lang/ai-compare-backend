// Routes خاصة بـ Admin - مصلحة لتعمل مع PostgreSQL
...
router.get('/users', checkAdminSecret, async (req, res) => {
  try {
    const result = await db.query('SELECT id, email, is_premium, created_at FROM users ORDER BY created_at DESC');
    res.json({ users: result.rows });
  } catch (err) {
    console.error('خطأ فـ جلب المستخدمين:', err);
...
router.post('/activate', checkAdminSecret, async (req, res) => {
  ...
  try {
    const result = await db.query('UPDATE users SET is_premium = 1 WHERE email = $1', [email]);
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'ما لقيتش هاد الإيميل' });
    }
    res.json({ success: true, message: 'تفعّل بنجاح: ' + email });
  } catch (err) {
    console.error('خطأ فـ تفعيل المستخدم:', err);
    res.status(500).json({ error: 'وقع مشكل فالسيرفر' });
  }
});
...
router.post('/deactivate', checkAdminSecret, async (req, res) => {
  ...
  try {
    const result = await db.query('UPDATE users SET is_premium = 0 WHERE email = $1', [email]);
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'ما لقيتش هاد الإيميل' });
    }
